...
public class UserListController {
    ...
    private void checkTextFieldUpdate(){
        ...
        if(statusButton == 1  && nameTextField.getText().length() >= 4 && loginTextField.getText().length() >=4 && passwordTextField.getText().length()>=6){
            boolean checkLogin = true;
            for(String s:dataTickets.getLoginList() )if(
                   s.equals(dataTickets.getUserListData().get(selectIndex-1).getLogin())){
              dataTickets.getLoginList().remove(dataTickets.getLoginList().indexOf(dataTickets.getUserListData().get(selectIndex-1).getLogin()));
          }
                for (String s: dataTickets.getLoginList()){
                    if (s.equals(loginTextField.getText())){
                        checkLogin = false;
